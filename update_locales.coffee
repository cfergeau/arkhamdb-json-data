        ...
        if stat.isDirectory()
            ...
            for pack_file in pack_files
                ...
                if !!fileContents.trim()
                    ...
                    result["#{path.join(file, pack_file)}"] = stripProps json, ['code', 'flavor', 'name', 'subname', 'customization_text', 'customization_change', 'text', 'traits', 'back_name', 'back_flavor', 'back_text', 'back_traits', 'slot']
        else
            #console.log "Reading regular #{path.join(localeRoot, file)}"
            fileContents = fs.readFileSync(path.join(localeRoot, file), 'UTF-8')
            if !!fileContents.trim()
                json = JSON.parse fileContents
                result[file] = stripProps json, ['code', 'flavor', 'name', 'subname', 'customization_text', 'customization_change', 'text', 'traits', 'back_name', 'back_flavor', 'back_text', 'back_traits', 'slot']
    result
